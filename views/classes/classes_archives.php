...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['restore'])) {
        $restoreRes = $db->query("UPDATE classes SET df = 0 WHERE topicname = $1", [$class]);
        if ($restoreRes) {
            $success = true;
    ...
    // Delete
        ...
        $confirmDelete = true;
        $res = $db->query("SELECT * FROM participantclassattendance WHERE topicname = $1", [$class]);
        if (pg_num_rows($res) > 0) {
            $warning = true;
        }
        $res = $db->query("SELECT * FROM facilitatorclassattendance WHERE topicname = $1", [$class]);
        if (pg_num_rows($res) > 0) {
            $warning = true;
        }
        $res = $db->query("SELECT * FROM classoffering WHERE topicname = $1", [$class]);
        if (pg_num_rows($res) > 0) {
            $warning = true;
        }
        $res = $db->query("SELECT * FROM curriculumclasses WHERE topicname = $1", [$class]);
        if (pg_num_rows($res) > 0) {
            $warning = true;
        }
    }
    // Confirm Delete
        ...
        // Remove actual curriculum-class connection
            ...
            $fullDeleteRes = $db->query("DELETE FROM classes WHERE topicname = $1", [$class]);
            if ($fullDeleteRes) {
                ...
                <form class="card warning-card" method="post" action="/classes/archive">
                    ...
                    <div class="card-body">
                        ...
                        if(isset($warning)) {
                            ...
                            echo "You are about to fully delete class \"$class\". Are you sure you want to fully delete this class?";
                        }
                        ?>
                    </div>
                        ...
                        <div class="card-footer d-flex flex-row justify-content-center">
                            <a>
                                <form class="mb-0" method="POST" action="/classes/archive">
                                    ...
                                    <input type="hidden" value="<?= $r['topicname'] ?>" name="class"/>
                                </form>
                            </a>
