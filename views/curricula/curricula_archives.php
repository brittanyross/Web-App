...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['restore'])) {
        $restoreRes = $db->query("UPDATE curricula SET df = 0 WHERE curriculumname = $1", [$curriculum]);
        if ($restoreRes) {
            $success = true;
    ...
    // Delete
        ...
        $confirmDelete = true;
        $res = $db->query("SELECT * FROM participantclassattendance WHERE curriculumname = $1", [$curriculum]);
        if (pg_num_rows($res) > 0) {
            $warning = true;
        }
        $res = $db->query("SELECT * FROM facilitatorclassattendance WHERE curriculumname = $1", [$curriculum]);
        if (pg_num_rows($res) > 0) {
            $warning = true;
        }
        $res = $db->query("SELECT * FROM classoffering WHERE curriculumname = $1", [$curriculum]);
        if (pg_num_rows($res) > 0) {
            $warning = true;
        }
        $res = $db->query("SELECT * FROM curriculumclasses WHERE curriculumname = $1", [$curriculum]);
        if (pg_num_rows($res) > 0) {
            $warning = true;
        }
    }
    // Confirm Delete
        ...
        $res = $db->query("DELETE FROM participantclassattendance ".
            "WHERE curriculumname = $1 ", [$curriculum]);
        ...
        // Remove actual curriculum-class connection
            ...
            $fullDeleteRes = $db->query("DELETE FROM curricula WHERE curriculumname = $1", [$curriculum]);
            if ($fullDeleteRes) {
...
?>
    ...
    <div style="width: 100%">
        ...
        <div class="d-flex flex-row justify-content-center flex-wrap">
            <?php
            // Confirm Full Delete
            if (isset($confirmDelete) && $confirmDelete && isset($curriculum)) {
                ?>
                <form class="card warning-card" method="post" action="/curricula/archive">
                    ...
                    <div class="card-body">
                        ...
                        if(isset($warning)) {
                            ...
                            echo "You are about to fully delete curriculum \"$curriculum\". Are you sure you want to fully delete this curriculum?";
                        }
                        ?>
                    </div>
                    ...
                    <input type="hidden" value="<?= $curriculum ?>" name="curriculum"/>
                </form>
                <?php
            }
            else {
                while ($r = pg_fetch_assoc($result)) {
                    ...
                    <div class="card text-center result-card">
                        <div class="card-body">
                            ...
                            <h6 class="card-subtitle text-muted"><?= $r['sitename'] ?></h6>
                        </div>
                        <div class="card-footer d-flex flex-row justify-content-center">
                            <a>
                                <form class="mb-0" method="POST" action="/curricula/archive">
                                    ...
                                    <input type="hidden" value="<?= $r['curriculumname'] ?>" name="curriculum"/>
                                </form>
                            </a>
