...
$type = isset($curricula) ? $curricula['sitename'] : '';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($valid) {
        if ($isEdit) {
            $res = $db->query("UPDATE curricula SET curriculumname = $1, sitename = $2, " .
                "missnumber = $3 WHERE curriculumname = $4", [$name, $type, $miss, $curriculaName]);
        } else {
            $res = $db->query("INSERT INTO curricula (curriculumname, sitename, missnumber) VALUES ($1, $2, $3);",
                [$name, $type, $miss]);
        }
        if ($res) {
        ...
        $curricula = pg_fetch_assoc($db->execute("get_curriculum", [$curriculaName]));
    } else {
        $success = false;
